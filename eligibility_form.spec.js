...
const URL = 'https://hiringdashboard.orbit-ed.com/parco-program-2026/apply-now';

test.describe('PARCO MTP 2026 - Eligibility Form', () => {

  test('Page loads and title is correct', async ({ page }) => {
    await page.goto(URL);
    await expect(page).toHaveTitle(/Parco MTP 2026/);
  ...
  test('Accepts valid input in personal information fields', async ({ page }) => {
    await page.goto(URL);
    await page.getByLabel(/First name/i).fill('Ahmed');
    ...
    await page.getByLabel(/Email/i).fill('ahmed.khan@example.com');
    await page.getByLabel(/City/i).fill('Islamabad');

    await expect(page.getByLabel(/First name/i)).toHaveValue('Ahmed');
    await expect(page.getByLabel(/Last name/i)).toHaveValue('Khan');
    await expect(page.getByLabel(/Email/i)).toHaveValue('ahmed.khan@example.com');
  });
